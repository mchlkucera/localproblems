...
| `reddit-search` | demand | Posts in four Czech subreddits matching pain terms — targeted complaint language rather than a firehose. | **3** `11100` | **0** | 200 · **WORKS** | `SCRIPTED-SILENT` | **CORRECTED 2026-08-21 — this cell used to read "the `.rss` + descriptive-UA mitigation works; no 429 seen", and that was wrong.** `data/feed_health.json` was right: *"transport: HTTP 429 — partial: 13 of 16 failed"*. Reddit states its budget on every reply, including the 429 — `x-ratelimit-used: 1`, `x-ratelimit-remaining: 0.0`, `x-ratelimit-reset: N` — and the budget for an unauthenticated client is **one request per clock-aligned minute**. Four back-to-back `.rss` GETs with the descriptive UA measured 200/429/429/429. Fixed by asking for less: `scripts/fetch_reddit.sh` now issues **2 requests instead of 20** (one merged multireddit `r/czech+Brno+Prague+czechia`, `limit=100`) and waits out the server's own `x-ratelimit-reset` rather than a fixed sleep. Re-measured after the fix: 2 of 2 at HTTP 200, 200 entries, **zero 429**. | Same axes as `suggest`; ranked above `reddit-new` because the pain-term filter is the precision half — the demand-signals lesson was that an unfiltered firehose drowns real pain in engagement. |
| `reddit-new` | demand | Every new post across four Czech subreddits as an Atom firehose, 25 entries per sub per pull. | **3** `11100` | **0** | 200 · **WORKS** | `SCRIPTED-SILENT` | **CORRECTED 2026-08-21 — "Nothing technical" was wrong.** Same 429 as `reddit-search` (`feed_health.json`: *"3 of 4 failed"*), same cause, same fix — see that row. The old script's arithmetic explains the ledger exactly: 20 requests at `sleep 10` spans ~4 one-request windows, and the ledger recorded exactly 4 successes. | Same axes; the firehose is the recall half and needs the pain filter downstream or it will dominate the demand ledger — the source-imbalance failure recorded in the wave-3 catalog notes. |
...
| `reddit-new` | `GET https://www.reddit.com/r/czech/new.rss` (UA `localproblems-register/1.0 …`) | 200 | 0.954 s | 43,673 | **Yes.** 25 `<entry>` elements. First: *"Zkušenosti s Rohlíkem"*, link `…/comments/1vte3hl/…`. No 429. **The measurement is real; the conclusion drawn from it was not.** One request per probe is precisely the workload this limiter never rejects — the budget is one request per minute, so a single-URL probe can *never* observe the 429 that the 20-request fetcher hit. Re-probed 2026-08-21 back-to-back: 200, then 429, 429, 429. |
| `reddit-search` | `GET https://www.reddit.com/r/czech/search.rss?q=nefunguje&restrict_sr=1&sort=new` (same UA) | 200 | 0.742 s | 69,438 | **Yes.** 25 entries. First: *"Filtrování reklam a videa na webu"*. No 429 despite following `reddit-new` by 40 s. **The 40 s gap is why it passed, not evidence that no limit exists** — it crossed a minute boundary. Measured 2026-08-21: `x-ratelimit-remaining: 0.0` was already returned on the *first* of those two requests. |
