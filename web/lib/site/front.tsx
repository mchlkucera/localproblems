// The front page, shared by its two STATIC routes: app/(site)/page.tsx
// (/, by opportunity) and app/(site)/by-category/page.tsx
// (/by-category). Neither reads `searchParams`, so both render at
// build time; the grouping is the route, never a query string (audit B1).
//
// The register as something you read through (owner, 2026-09-15:
...
/** The lede, verbatim: it is also the page's description. */
const LEDE =
  "The register looks for problems where three things meet: what people want, what government wants, and what already works abroad.";

/** Both routes share the title and the description; only the grouping differs. */
export const frontMetadata: Metadata = {
  title: "Czech problems worth solving — localproblems.org",
  description: LEDE,
};
...
export type Item = { p: Problem; href: string; solution: string; brief: string | null; goodFor: string | null; draftLaw: string | null };

export function item(p: Problem): Item {
  ...
  // `draft_law` (2026-09-16): set only where the main pain depends on a law
  // that is not passed yet — the row then carries the "Draft law" badge.
  const h = p as { brief?: string; good_for?: string; draft_law?: string };
  return {
    p,
    href: `/problem/${p.region}/${p.id}`,
    solution: plain(p.solution),
    brief: typeof h.brief === "string" && h.brief.trim() ? h.brief.trim() : null,
    goodFor: h.good_for?.trim() || null,
    draftLaw: typeof h.draft_law === "string" && h.draft_law.trim() ? plain(h.draft_law) : null,
  };
}

// ---- grouping (owner, 2026-09-15: "bring back the grouping selector") -------
// Plain links between two static routes, no JS, no query string. Every grouping
// yields the same shape — a rail label, a count, entries in register order — so
// the sticky rail works identically for both. Deadline grouping was removed
// (owner, 2026-09-16: "Remove deadline sort").

const GROUPINGS = [
  { key: "opportunity", label: "By opportunity", href: "/" },
  { key: "category", label: "By category", href: "/by-category" },
] as const;
export type GroupKey = (typeof GROUPINGS)[number]["key"];
export type Group = { id: string; label: string; items: Item[]; category?: string };
...
export function byOpportunity(items: Item[]): Group[] {
...
// in /lab/paper"): the per-category isometric drawing from lib/art — paper
...
/** THE ONE BADGE (owner, 2026-09-16: "add some badge to all problems that are
    'probably': based on a law that's not yet released"). A neutral outlined
    gray pill — uncertainty, so neither the favour teal nor the warm
    time-pressure hues. It has a card, so it takes a keyboard stop (tabIndex
    0) as well as hover and tap; the card names the draft and says what it
    means, in the page's tooltip style. */
function DraftLawBadge({ id, line }: { id: string; line: string }) {
  return (
    <span className="lf-m lf-badge-host" tabIndex={0} aria-describedby={id}>
      <span className="lf-badge">Draft law</span>
      <span className="lf-m-tip" id={id} role="tooltip">
        <strong>{line}</strong>
        Based on a law that is not passed yet, so this may change.
      </span>
    </span>
  );
}
...
export function FrontPage({ group }: { group: GroupKey }) {
