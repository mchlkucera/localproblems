...
Why now: NZÚ volumes keep the long tail of installers at capacity, and the German comparison is direct — autarc (YC S24, Berlin) built exactly this stack for a market whose installer workflows and subsidy bureaucracy mirror the Czech setup, and reached ~30 people on it. The workflow is subsidy-shaped, so a CZ product must be built around NZÚ specifically; foreign tools do not transfer without localization, which protects a local entrant.
