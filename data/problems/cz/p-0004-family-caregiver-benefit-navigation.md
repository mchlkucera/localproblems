...
- type: tender
  ...
  note: 'ted-402149-2026 (context): MPSV ''IT delivery III'' framework ~€74.7M plus a dozen
    ...
    Rescored 2026-09-19: the rungs named above are retired. The framework buys the ministry''s
    own back-office IT, not help for a family filing a claim, so it is not this job and backs
    no score: dims set to [].'
  date: '2026-06-11'
  dims: []
- type: statistic
...
- None of these dates puts a duty on a family: the Act changes what the state pays, and the bill is not law [S7,S8].
...
- The ministry's €74.7M benefits IT framework of June 2026 buys none of this [S4].
...
2. Contact the free carers' advice services and ask them to pass on the families whose claims they cannot carry through. They give advice or write an appeal, and stop there; see [Market gap](#competition). The claim still has to be filed, chased and defended, which is the work a family would pay someone to do, and the advisers are glad to have somewhere to send them.
...
4. Charge for the outcome, a flat or success fee on the award, because nobody in Czechia sells a family the claim itself. An online law firm already sells fixed-price consultations but sends care-allowance cases to court representation; see [Market gap](#competition). What no one sells is the whole claim, from the application through the home visit to the appeal, so price that.
...
2026-09-19 · rescored to the 2026-09-19 ladders — Why now 1 → 0; Willing to pay 1 → 0; score 7 → 5, still FAIR. Why now: the old 1 was the retired freshness point on a deadline part of 0. The one regulation that backs a score by type, Act No. 360/2025 [S7], raises the two lowest grades: it changes what the state pays and puts no dated duty on a family or on anyone this service would sell to, so it scores 0 on this ladder. MPs' bill [S8] is a draft and carries `dims: []`. Willing to pay, tagging pass first: every source on file was read for someone paying for this exact job, filing a family's claim, preparing it for the home visit and running the appeal, or that work bought in. None does, so nothing was restated or tagged, and the old 1, which rested on the ministry's IT framework as adjacent spend, falls to 0. Read and not counted, and why: the online law firm's 390 CZK consultation is general legal advice whose care-allowance page routes to court representation, not the claim [S9]; the registered counselling service writes appeals for free, which is no payment, and only the appeal [S6,S9]; the ministry's €74.7M IT framework buys the state's own back office [S4]; and the allowance itself is the money at stake, paid to the person cared for, not a payment for this help [S2,S5]. S4 is set to `dims: []`, since it buys none of this job, and its note, which named the retired rungs, gained a rescore line. S2 keeps its default tag as public money nearby. Body: under Willing to pay, the framework item now says it buys none of this [S4]; Why now gained a bullet saying none of its dates puts a duty on a family [S7,S8]. The two links to the section now named Market gap carry that name. Same result as the worksheet, which offered `dims: []` on S2 and S4 as optional; S2 was left as it was.
