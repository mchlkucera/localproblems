...
**A deep sweep is a MODE of the scan feeds, not a third loop** (owner mandate 2026-08-24,
after an elderly-care capacity problem reached the register by founder anecdote rather
than through the pipeline — the monthly scans sweep broadly across categories, and nothing
could go deep on one topic on demand). A sweep is an attended, topic-scoped pass of the
four registered agent scan sources — `arb-scan`, `demand-scan`, `reg-scan`, `dotace-scan`:
input one topic, output records across all four evidence streams for it, through the same
source values, id prefixes and ledgers, at the same evidence bar and through the same
`normalize.py --complete` gate as a monthly harvest. No new feed, no new id namespace, no
schema change. Like INGEST it carries no problem judgment and NEVER commits; PROCESSING
picks its records up exactly as it picks up any other harvest. Launchable by Claude end to
end from `pipeline/SWEEP.md` alone, on demand, never on a schedule.

**The monthly broad passes carry mandatory source checklists** (post-mortem 2026-08-25:
the scans missed in-scope events because nothing forced each pass through each source in
its remit). Every broad pass of the four scan feeds walks every source on its feed's
checklist in `pipeline/SCANS.md`; a source not visited that pass is a named coverage gap
in the run manifest — silence is forbidden. Deep dives remain `SWEEP.md`'s job.
...
| `/problem/[region]/[id]` | one record page for every **non-rejected** problem: head (art, title, brief, facts), The opportunity · Suggested solution · Why now · Willing to pay · Validated abroad · Competition · Execution difficulty · Suggested first moves, each an outline on the page with a Read more sheet (older anchors such as `#problem`, `#who-pays`, `#how-big` stay as aliases); the rail's contents card with section scores ("Opportunity n/12") and its evidence card; source pills that peek; the sources drawer (`#sources`, rows `#s1…sN`); a footer with the corrections link. Rejected records return 404 (below). |
...
| `/sources` | PRIVATE, built only with `LP_ADMIN=1` (else 404): the feeds registry beside the observed health ledger, in the signal ledgers' compact layout (modern design since 2026-09-17). |
| 404 | the modern 404: "Record not found", check the address, back to Problems. |

Every route lives in `app/(site)`, `/sources` included. `web/app/lab/`
...
- **Design:** the `design-language` skill is binding. **It is the modern design as of
  ...
  **Locks:** `web/app/(site)/styles/tokens.css` is a verbatim copy of
  `skills/design-language/assets/tokens.css`; `check-css` asserts it. The gazette
  stylesheet and its lock were deleted with the last gazette page (2026-09-17).
  `check-site` asserts the static contract above (no request-time APIs, the client
...
- **Client-side JavaScript** beyond the named progressive enhancements in §5: the
  `PeekHover` source-peek script and native popovers. No hydration-dependent UI: a page must read fully with JS off, and no
...
5. **The repo is lean:** only SPEC / SCORING / CONVENTIONS + `pipeline/` (the two agent
   ...
   **There is no launchd plist.** Earlier versions of this clause and of §12 counted a
   ...
   `pipeline/` has only ever held its agent entry points — `INGEST.md`, `PROCESS.md`,
   since 2026-08-24 `SWEEP.md`, and since 2026-08-25 `SCANS.md` (§2). The scheduler is a future
...
| **`pipeline/`** | **the agent entry points** — `INGEST.md` (fetch + normalize, hourly-ish), `PROCESS.md` (match → deploy, on-demand), `SWEEP.md` (topic-scoped deep sweep of the scan feeds, on-demand — §2) and `SCANS.md` (the scan feeds' mandatory monthly source checklists — §2, added 2026-08-25). Operational form of §2. Those four files are all it holds; there is no launchd plist (§9.5). |
...
| `skills/design-language/` | the design system — binding; the modern design since 2026-09-16 (owner). Exact values for the front page, row card and ledgers live in `web/app/(site)/DESIGN.md`, which the skill points to. `assets/tokens.css` is locked to the site's tokens |
