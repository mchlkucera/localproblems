...
brief: 'Doctors type reports as free text, then other staff re-read them by hand [S1,S3]. Two state grants pay named hospitals to upgrade, but both close to applications in December [S8,S18].'
...
process:
  summary:
    ...
    after: 'The doctor fills a report template in the hospital system, so the report is data as it is written, and the coder and the documentarian confirm the insurer codes and the registry entry proposed from that data.'
  steps:
  - who: Doctor
    today: 'Writes the report as free text'
    known: documented
    cites: [1, 3]
    change: changes
    after: 'Fills a report template in the hospital system'
  - who: Coder
    today: 'Re-reads the report for the insurer''s codes'
    known: documented
    cites: [3]
    reenters: true
    change: changes
    after: 'Confirms the proposed insurer codes'
  - who: Documentarian
    today: 'Re-reads it for the cancer registry'
    known: documented
    cites: [2]
    reenters: true
    change: changes
    after: 'Confirms the proposed registry entry'
  - who: '?'
...
- type: news
  ...
  dims: []
- type: subsidy
  name: "IROP calls 78 and 79 — eHealth, about 2.1bn CZK together"
  gist: "call 79 and its companion, call 78"
  why: "The eHealth call in S8 has a companion, IROP call 78, with 471.9M CZK for its own list of named providers, up to 9.5M CZK each; both close on 2 December 2026 and together hold about 2.1bn CZK."
  url: https://irop.gov.cz/cs/vyzvy-2021-2027/vyzvy/79vyzvairop
  note: 'dotace-irop-78-79-ehealth (dotace-scan, 2026-09-18), linked in MATCH 2026-09-18.
    ...
    cap 9.5M CZK of eligible costs per healthcare provider, 5M CZK per ambulance service; call
    79 (06_23_079): 1,144,629,052 + 490,555,308 = 1,635,184,360 CZK, cap 28M CZK, which
    confirms S8 unchanged; together 2,107,039,100 CZK (call texts "Text 78/79 výzvy eHealth k 26.
    5. 2026"; totals match the MS2021+ XML). Both continuous, applications 28 Nov 2023 to 2 Dec
...
- Named hospitals can apply to two state grants until 2 December 2026 [S8,S18].
