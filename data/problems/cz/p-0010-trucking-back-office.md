...
brief: 'A dispatcher takes each load by phone, and the office runs delivery papers and invoices through e-mail and old dispatch software [S2]. From July 2027, authorities can''t demand paper if a certified digital record exists [S3].'
...
- type: regulation
  ...
  note: 'reg-efti-freight: eFTI Regulation (EU) 2020/1056 — from 9 Jul 2027 authorities in
    ...
    estimates up to €1bn/yr sector savings. Corrected 2026-09-19: the duty binds the authorities,
    which must accept the digital record, not the hauliers, and paper stays allowed, so nothing
    expires for the haulier. On the 2026-09-19 ladder it backs Why now 1, a dated duty on someone
    else that reaches the buyer indirectly, not the deadline point it carried under v1.'
  date: '2027-07-09'
...
Who pays: Hauliers already pay for back-office software, and one Czech product has over 700 transport firms on it, but no Czech price is on file [S11].
...
2. Build the part that reads a delivered load's delivery note and CMR consignment note and turns them into an invoice ready to send. The driver photographs the papers where the load is delivered; the software reads them in Czech, drafts the invoice and hands it to the office to check and send. The Czech back office already on sale raises its invoices from the truck's tracking data rather than from the papers, and claims no certified digital format; see [Market gap](#competition). Build that format in from the start, because authorities must accept certified digital freight papers from the date under [Why now](#why-now).
3. Test whether software can take a Czech dispatcher's phone calls, by running a prototype phone agent against recorded calls from the firms you sat with. The closest foreign company already answers trucking firms' calls with AI phone agents, but it sells only in the US, as [Validated abroad](#validated-abroad) shows. The Czech back office already on sale does not answer the phone; see [Market gap](#competition). If the prototype handles Czech calls well, that is what keeps foreign sellers out, and it becomes the second thing you sell.
...
2026-09-19 · brief corrected (owner-approved) — Brief before, verbatim: "A dispatcher takes each load by phone, and the office chases delivery papers and invoices by e-mail [S2]. From July 2027, authorities can't demand paper if a certified digital record exists [S3]." After: "A dispatcher takes each load by phone, and the office runs delivery papers and invoices through e-mail and old dispatch software [S2]. From July 2027, authorities can't demand paper if a certified digital record exists [S3]." Why: "chases" was withdrawn from the title and the body on 2026-09-16 because [S2] does not support it, and the 2026-09-18 entry reported it still standing in the brief. The owner suggested "retypes" as the replacement; it was checked and not used, because [S2]'s note and the signal behind it (yc-hemut, re-read on this date) say only that Czech hauliers run "dispatch, documents and invoicing on phones, e-mail and legacy dispatch software", and the 2026-09-16 entry withdrew re-typing for the same reason. The data entry the signal mentions is what the US comparable automates, not a finding about Czech offices. So the brief now says what [S2] says: the office runs the papers and invoices through e-mail and old dispatch software, the wording `process.summary.today` already uses. No score, status, source, note, marker, title, solution, good_for or body sentence changed.

2026-09-19 · rescored to the 2026-09-19 ladders — Why now 3 → 1 and `score` 6 → 4, so the band moves FAIR → FAINT; Willing to pay stays 0. Why now was 3 as deadline 2 plus the freshness point, which is retired. The only dated instrument on file is the eFTI Regulation, from 9 July 2027 [S3]. It is enacted and inside 18 months, but it binds the authorities, which must accept a certified digital record, and not the hauliers this problem sells to, who may keep using paper. A duty on someone else that reaches the buyer indirectly is rung 1. No sanction on hauliers exists to test. The OP TAK grant's closing date is a grant date, not a deadline [S7]. [S3]'s note said the paper back office "acquires a regulatory expiry date"; the body already withdrew that claim on 2026-09-18, and the note now carries the correction. Tagging pass for Willing to pay: no source on file shows a Czech haulier paying a named amount for this job. The Czech back-office product has more than 700 transport firms on it, but its price is not on file [S11]; Datatruck's $99–500 a month is a US seller's price to US fleets, filed as a statistic [S10]; the grant pays half of a rural firm's software but has no price to lift [S7]; the road-freight market size is not a price [S9]. No receipt added; money stays 0, as the worksheet had it. A Czech list price for this job would give 1, and with the grant's lift 2. Why now prose re-read: it already says the rule binds the authorities, not the hauliers, so it stands. Willing to pay's answer sentence said hauliers pay, with nothing priced beside it; it now adds that no Czech price is on file, so the words match a score of 0 [S11]. `[Competition](#competition)` became `[Market gap](#competition)` in moves 2 and 3. No other score, status, entry or body sentence changed.
