...
- type: tender
  ...
  note: 'ted-385664-2026: obec Petrovice u Karviné awarded ~€278k design-and-build for ''Komunitní
    ...
    Rescored 2026-09-19: the money rungs this note names are retired. A design-and-build award
    buys the generation, not member billing, so it is public money nearby and earns no point;
    money now rests on the signed-contract price receipt, S16.'
  date: '2026-06-05'
...
- type: contract
  ...
  note: 'hlidac-38667544: MŠ Dětská, Liberec signed sharing contract č. 32 with Energetické
    ...
    Rescored 2026-09-19: "recurring annual spend" is a retired money rung. This contract buys the
    same job as the care home''s, but no amount for it is on file, so it cannot be a price
    receipt; it stays public money nearby.'
  date: '2026-06-29'
...
- type: price
  ...
  why: "The same care-home contract read clause by clause: its 960,000 CZK pays for the shared electricity and the community's administration fee together, and nothing on it splits the two. The fee pays for the hub paperwork, data processing and billing, which is this job bought in."
  note: 'Price receipt drawn from the contract already on this ledger (hlidac-38899662,
    ...
    Rescored 2026-09-19: restated from type contract to a signed-contract price receipt and
    tagged dims: [money], replacing the dims: [] above. It counts because it buys this job:
    clauses 2.2, 2.3 and 3.1 charge a separate administration fee set by the community price
    list (S18), whose purpose is EDC administration, data processing and billing each member,
    the work the software does. The amount also covers the shared electricity, 0.484 CZK/kWh
    against the fee''s 1.516, and the split in crowns is unknown, which why says. Paid on
    2026-07-01, inside 24 months of updated: money 2.'
  date: '2026-07-01'
...
- None of these dates is a deadline a community or its members must meet: the September 2026 rules allow bigger groups, and the fund's date closes a grant [S11,S12].
...
- One care home's open-ended contract pays for the electricity and the fee together [S6].
...
The contracts behind the other two items:

- The care home is Dům seniorů Františkov in Liberec. Its contract, č. 58, pays for the shared electricity, owed to the producers, plus the admin fee, and names the price list as part of it [S6]. Its value is in the table of what one buyer pays; it states no volume, so how that value splits between the two is unknown [S6].
...
2. Next, call the South Moravian energy agency, which already runs sharing for a private firm in another community. It sells that work as a service, not as software; see [Market gap](#competition). Abroad, energy service firms like it buy white-label billing software and offer it under their own name; see [Validated abroad](#validated-abroad). Ask how it does the work today, and whether it would run its communities on your software.
...
2026-09-19 · rescored to the 2026-09-19 ladders — Why now 1 → 0; Willing to pay stays 2; score 9 → 8, still STRONG. Why now: the old 1 was the retired freshness point on a deadline part of 0. The one regulation on file, ERÚ decree 132/2026 Sb. [S12], allows sharing groups of up to 100 supply points and puts its only duty on EDC, the data hub; a rule that only permits sets no deadline for the buyer. The fund's 31 December 2027 close [S11] is a grant date, which this ladder does not read. Willing to pay, tagging pass first: every source on file was read for someone paying for this exact job, which is billing each member from the hub's data, or that work bought in. Counted: the care home's contract č. 58 with Energetické společenství Liberec, 960,000 CZK, signed 1 July 2026 [S6]. Its clauses charge a separate administration fee, set by the community's price list, for the hub paperwork, data processing and billing [S18]: this job, done by the community and bought by a public body. By the owner-approved rule for this record's judgement call (it counts only if it buys this job), it counts. It is restated as the price receipt S16: payer Dům seniorů Františkov, 960,000 CZK, one-off, basis signed-contract, dated 2026-07-01, `dims: [money]`, replacing `dims: []`. Its amount also covers the shared electricity, and its why says so. A paid receipt inside 24 months is rung 2, so no public-money lift is needed. Not counted, and why: the Liberec schools series buys the same job but has no amount on file [S7,S14]; the South Moravian agency's sharing-administration contract states no value [S8]; the Pardubice hospital group's order is a membership contribution naming neither electricity nor administration [S10,S17]; the Petrovice award buys generation [S5]; EDC's new IT system is the hub itself [S15]; KOMUNERG is a grant [S11]; and the Liberec price list prices exactly this work but stays a contract, because the price vocabulary has no per-kWh unit [S18]. S6 now carries `dims: []`, so the one contract is not counted twice, as the receipt and as public money nearby. Stale notes on S5 and S7, which named the retired money rungs, gained a rescore line; nothing in them was rewritten. Body: under Willing to pay, the 960,000 CZK figure left the list and the care-home bullet for its receipt, which the page draws in the table of what one buyer pays, and the [S16] markers there became [S6], since a price receipt is never cited from the body; Why now gained a bullet saying none of its dates is a deadline a community must meet [S11,S12]. The two links to the section now named Market gap carry that name. Same result as the worksheet.
