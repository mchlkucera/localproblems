...
sources:
  ...
  - type: regulation
    ...
    note: 'reg-vak-srazkove-vody-poplatek (reg-scan, 2026-09-03), read from the RIA attachment
      ...
      procedure; the RIA summary sheet gives effective 07/2027. It is a DRAFT: Why now is scored at
      1 because the forcing function is not yet law (it fails REAL) and the same deletion was
  ...
  - type: regulation
    ...
    note: 'veklep-KORNDXBH65S3 (scripted veklep feed, 2026-09-02). The bill''s status receipt: with S3
      it backs Why now at rung 1, a draft that is not yet law.'
    date: '2026-08-27'
  ...
  - type: price
    ...
    note: 'Registr smluv 8041799 (contract 01419/2018), "Smlouva o dílo na zhotovení pasportu ploch
      ...
      the manual equivalent is rung 1. Signed in 2019, more than 24 months before `updated`, so on
      the 2026-09-19 ladder it is an asking receipt, not a paid one (rescore, 2026-09-19).'
    date: '2019-01-14'
...
Because roads and homes were never billed, an operator has no declared area on file for them [S8]. And no Czech firm sells the measuring; see [Market gap](#competition) [S13].
...
Who pays: One Czech city paid for a surface register in 2019, and water companies buy billing software for the charge [S13].
...
Kolín's price is in the table of what one buyer pays [S13]. The billing software takes whatever area the operator keys in; see [Market gap](#competition) [S13]. If the draft passes, two sides would need the new areas measured [S3]:
...
2026-09-19 · rescored to the 2026-09-19 ladders — Why now 2 → 1 and the total 9 → 8; the band stays STRONG. The old 2 was deadline 1 plus the freshness point, which is retired. The charge rests on the agriculture ministry's draft [S3,S4] and on an EU directive still awaiting transposition [S5]; both fail REAL (not yet binding law), so the new ladder gives rung 1. Willing to pay stays 1. Tagging pass: the one receipt on file for this job is Kolín's register of billable surfaces, 1,449,919 CZK one-off, paid by Město Kolín, signed 14 January 2019, basis signed-contract [S9]. It was already tagged money, but it is older than 24 months, so it counts as an asking receipt (rung 1), not a paid one. The state loan call [S10] pays for treatment plants, not this job, and stays untagged. The gap check's full-text searches of the contracts register found no other contract for this job [S13]. No receipt was added. Notes: S3 no longer names the deadline point, S4 no longer claims the freshness point, and S9 says why a 2019 contract is rung 1. Body: the 3 `[Competition](#competition)` links now read `[Market gap](#competition)`. Willing to pay's opening sentence dates Kolín's payment to 2019 instead of saying "some money is spent already", which read as paying now. No other score, status, source, marker or `entry` gate changed.
