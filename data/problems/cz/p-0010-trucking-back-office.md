...
- name: TruckManager / TruckAgenda
  ...
  evidence: 'The haulier''s own back office, sold in Czech to exactly these small hauliers by 1.
    ...
    does not answer the phone, it builds the invoice from telematics rather than by reading the
    document, and it claims no certified electronic freight rail for 2027 — but it sells this.'
- name: Ringil
  ...
  evidence: 'Used by Škoda and Notino, Ringil sells four modules — transport management, delivery
    ...
    Máte přístup zdarma"): an RFQ inbox, one-click bidding, a record of that one shipper''s
